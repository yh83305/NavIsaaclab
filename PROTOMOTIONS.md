...
### Crowd navigation dependencies

The CrowdSim NavDP and FLUX integrations use the official repositories as Git
submodules. Clone this repository together with both dependencies using:

```bash
git clone \
  --recurse-submodules=NavDP \
  --recurse-submodules=FLUX \
  <repository-url>
```

For an existing clone, initialize or update them with:

```bash
git submodule update --init NavDP FLUX
```

The explicit paths are intentional: the official NavDP revision contains an
unconfigured nested gitlink that is not required by CrowdSim, so a blanket
recursive submodule update should not be used.

The parent repository pins exact NavDP and FLUX commits so experiments remain
reproducible. Checkpoints and downloaded benchmark assets are not stored in the
submodules and must still be downloaded separately.
