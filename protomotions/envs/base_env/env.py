...
class BaseEnv:
    ...
    def step(self, action: Tensor):
        ...
        profile = os.environ.get("CROWDSIM_PROFILE_BASE_ENV", "0") == "1"
        ...
        simulator_elapsed = time.perf_counter() - started if profile else 0.0

        started = time.perf_counter() if profile else 0.0
        self.post_physics_step()
        post_elapsed = time.perf_counter() - started if profile else 0.0
        ...
        started = time.perf_counter() if profile else 0.0
        obs = self.get_obs()
        observation_elapsed = time.perf_counter() - started if profile else 0.0
        if profile:
            stats = getattr(self, "_crowdsim_step_profile", None)
            if stats is None:
                stats = self._crowdsim_step_profile = {
                    "count": 0,
                    "simulator_step": 0.0,
                    "post_physics": 0.0,
                    "get_obs": 0.0,
            ...
            stats["count"] += 1
            stats["simulator_step"] += simulator_elapsed
            stats["post_physics"] += post_elapsed
            stats["get_obs"] += observation_elapsed
            if stats["count"] % 25 == 0:
                count = stats["count"]
                print(
                    "[BaseEnv profile] mean ms: "
                    + " ".join(
                        f"{name}={1000.0 * stats[name] / count:.1f}"
                        for name in ("simulator_step", "post_physics", "get_obs")
                    ),
                    flush=True,
                )
    ...
    def post_physics_step(self):
        ...
        # Deployment/data collection never consumes the ProtoMotions reward.
        # Keep observations and terminations authoritative, but avoid the
        # expensive reward graph when the persistent environment opts in.
        if getattr(self, "_crowdsim_fast_runtime", False):
            self.rew_buf.zero_()
        else:
        ...
        if not getattr(self, "_crowdsim_fast_runtime", False):
